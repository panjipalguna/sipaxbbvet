			...
			'buttons'  => [
				'leadStatus' => function ($url, $model) {
					...
					if($user->status==1){
						...
						return Html::a('<span class="fa fa-check"></span>', $url, ['class' => 'btn btn-primary', 'data-toggle'=>'tooltip', 'data-placement'=>'top', 'title'=>'User Aktif']);
					}else{
						$url = Url::to(['status', 'id' => $model->id, 'q' => 0]);
						return Html::a('<span class="fa fa-remove"></span>', $url, ['class' => 'btn btn-inverse', 'data-toggle'=>'tooltip', 'data-placement'=>'top', 'title'=>'User Non Aktif']);
					}
				},
